...
public class MultiCsv {
	...
	public static GIS_project_obj csvFiles2project(String folder_path) {
		...
		for (int i = 0; i < csvFiles.length; i++) {
			GIS_layer_obj layer = c2g.csv2GisLayer(csvFiles[i].getName());
			project.add(layer);
		}
	...
	public static void GisProject2kml(GIS_project_obj project) {
        ...
        try{
        	int kml_name = 0;
        	boolean check = new File("//", Integer.toString(kml_name)).exists();
        	while (check) {
        		kml_name++;
        		check = new File("//", Integer.toString(kml_name)).exists();
        	}
	        FileWriter fw = new FileWriter("multiCsv" + kml_name + ".kml");
